...
namespace {
	...
	ll get(int v, int i) {
		return MA(MM(dp[v][min(int(dp[v].size()) - 1, i)], mz[v]), az[v]);
	}
	void dfs(int u) {
		if (ch[u].empty()) {
		...
		for (auto& v : ch[u]) {
			dfs(v);
			dp[v].push_front(MD(MS(0, az[v]), mz[v]));
			if (w == -1) w = v;
		...
		for (auto& v : ch[u]) {
			...
			az[u] = MA(az[u], MM(get(v, n), t));
			for (int i{0}; i < dp[v].size() - 1; ++i) {
				dp[u][i] = MS(dp[u][i], MD(MM(get(v, n), t), mz[u]));
				dp[u][i] = MA(dp[u][i], MD(MM(get(v, i), t), mz[u]));
			}
		}
	}
	void solve() {
